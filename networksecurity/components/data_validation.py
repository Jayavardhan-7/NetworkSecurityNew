...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            raise NetworkSecurityException(e,sys)
    def numerical_column_check(self,dataframe:pd.DataFrame,)->bool:
        number_of_columns=len(self._schema_config['numerical_columns'])
        length=len(dataframe.select_dtypes(include=['number']).columns)
        logging.info(f"Required number of numerical columns:{number_of_columns}")
        logging.info(f"Data frame has numerical columns:{length}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status = self.numerical_column_check(dataframe=train_dataframe)
            if not status:
                error_message=f"Train dataframe does not contain all numerical columns.\n"
            status = self.numerical_column_check(dataframe=test_dataframe)
            if not status:
